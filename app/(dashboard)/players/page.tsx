...
export default async function PlayersPage({ searchParams }: PageProps) {
  ...
  // IDs des communautés où le user courant est membre (pour afficher les communautés partagées)
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {q.length >= 2
              ...
              : `${users.length} joueur${users.length > 1 ? "s" : ""} inscrits`}
          </p>
        </div>
      </div>
      ...
      {users.length === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {users.map((user) => {
            ...
            // Communautés visibles : publiques ou partagées avec le user connecté
