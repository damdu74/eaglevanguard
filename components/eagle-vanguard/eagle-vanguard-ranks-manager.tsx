  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-2">
        {fnRanks.length === 0 && (
          <p className="text-sm text-muted-foreground text-center py-2">Aucune fonction.</p>
        )}
        {fnRanks.map(rank => (
          <div key={rank.id} className="rounded-md border">
            {editingId === rank.id ? (
              <div className="flex items-center gap-1.5 px-3 py-2 flex-wrap">
                <Input value={editName} onChange={e => setEditName(e.target.value)} className="w-36 h-7 text-xs" />
              ...
              <div className="flex items-center justify-between px-3 py-2">
                ...
                <div className="flex items-center gap-1">
                  <Button size="sm" variant="ghost" className="h-6 w-6 p-0 text-xs" onClick={() => startEdit(rank)}>✎</Button>
                  <Button size="sm" variant="ghost" className="h-6 w-6 p-0 text-destructive hover:text-destructive" onClick={() => deleteFn(rank.id)}>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-1.5">
            <Input value={newName} onChange={e => setNewName(e.target.value)} onKeyDown={e => e.key === "Enter" && createFn()} placeholder="Nom" className="flex-1 h-7 text-xs" />
            <input type="color" value={newColor} onChange={e => setNewColor(e.target.value)} className="h-7 w-8 cursor-pointer rounded border border-border" />
            <Button size="sm" className="h-7 px-2" onClick={createFn} disabled={creating || !newName.trim()}>
