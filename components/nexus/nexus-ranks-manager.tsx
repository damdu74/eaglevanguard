...
import { NEXUS_PERMISSIONS, NEXUS_PERMISSION_LABELS, type NexusPermission } from "@/lib/permissions"
...
function RankSection({ title, description, category, ranks, setRanks }: {
  ...
  async function savePermissions(rank: NexusRank) {
    setSaving(rank.id)
    try {
      const res = await fetch(`/api/nexus/ranks/${rank.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  function togglePermission(rankId: string, perm: NexusPermission) {
              ...
              <>
                ...
                {category === "ROLE" && expanded === rank.id && (
                  <div className="border-t px-3 py-2 space-y-1.5">
                    ...
                    {NEXUS_PERMISSIONS.map((perm) => (
                      <div key={perm} className="flex items-center justify-between">
                        <span className="text-xs">{NEXUS_PERMISSION_LABELS[perm]}</span>
                        <Switch
